<template>
  <div page="测试页" :class="$options.name">
    ...
    <BaseSilenced imGroupId="yzb_dev_live_37" imUserId="yzb_dev_5" v-model="isShutUp" />
  </div>
</template>
  ...
  methods: {
    ...
    async getRemoteData() {
      try {
        ...
        console.log("get IP Err:", error.msg);
        if (!process.server) this.$confirm("Err:" +error.msg);
        return false;
      }
    },
